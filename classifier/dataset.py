...
from config import DataConfig, get_label_maps, REAL_CLASS_NAME, get_two_stage_label_maps


class WatermarkDataset(Dataset):
    def __init__(self, image_paths: List[str], labels: List[int], transform=None, augment=None):
...
def gather_image_paths_and_labels(data_config: DataConfig) -> Tuple[List[str], List[int]]:
    ...
    for method, dirname in data_config.method_dirs.items():
        ...
        print(f"  {method} ({dirname}): {len(fnames)}/{total} images (cap={cap})")
